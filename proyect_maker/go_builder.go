...
func JWTGoMakeCode(P Proyect, Obs []Object) {
	buildPath := "go_build"
	err := os.MkdirAll("./"+buildPath+"/models", os.ModePerm)
	check(err)

	err = os.MkdirAll("./"+buildPath+"/api", os.ModePerm)
	check(err)

	buildGoMain(buildPath, P, Obs)
	buildGoConnection(buildPath, P)
	buildGoStuffs(buildPath)
	buildGoModels(buildPath, Obs, P.Db)
	//buildJWTGoConfigFile()
	buildGoAPIEndPoints(buildPath, Obs)
}

func buildGoMain(buildPath string, P Proyect, Objs []Object) {
	f, err := os.Create("./" + buildPath + "/server.go")
	check(err)
	defer f.Close()
...
func buildGoConnection(buildPath string, P Proyect) {
	f, err := os.Create("./" + buildPath + "/models/db.go")
	check(err)
	defer f.Close()
	...
	case "POSTGRESQL":
		_, err = f.WriteString(strGoMongoDBConnection(P.Db_Data.Db_Name, P.Db_Data.DB_Collate))
	case "SQLSERVER":
		_, err = f.WriteString(" -- ")
	}
...
func strGoMongoDBConnection(DBname string, DBcoll string) string {
	strP := ""
...
func buildGoStuffs(buildPath string) {
	f, err := os.Create("./" + buildPath + "/models/stuffs.go")
	check(err)
	defer f.Close()
...
func buildGoModels(buildPath string, Objs []Object, db string) {
	for _, O := range Objs {
		buildGoModel(buildPath, O, db)
	}
}

func buildGoModel(buildPath string, O Object, db string) {
...
func strGoLogin(O Object) string {
	strP := ""
...
func strGoInsert(O Object) string {
...
func strGoMONGODBInsert(O Object) string {
	strP := ""
...
func strGoUpdate(O Object) string {
...
func strGoMONGODBUpdate(O Object) string {
	strP := ""
...
func strGoDelete(O Object) string {
...
func strGoMONGODBDelete(O Object) string {
	strP := ""
...
func strGoFindOne(O Object) string {
	strP := "\tstrSQL := " + `"` + "SELECT "
	strQ := ""
	strW := ""
...
func strGoMONGODBFindOne(O Object) string {
	strP := ""
...
func strGoFindAll(O Object) string {
	strP := "\tstrSQL := " + `"` + "SELECT "
	strW := ""
...
func strGoMONGODBFindAll(O Object) string {
	strP := ""
...
func strGoFindAllPag(O Object) string {
	strP := "\tstrSQL := " + `"` + "SELECT "
	strW := ""
...
func strGoMONGODBFindAllPag(O Object) string {
	strP := ""
...
func buildGoAPIEndPoints(buildPath string, Objs []Object) {
	for _, O := range Objs {
		buildGoAPIEndPoint(buildPath, O)
		if O.Is_Login {
			buildGoAPILogin(buildPath, O)
		}
	}
}

func buildGoAPILogin(buildPath string, O Object) {
	f, err := os.Create("./" + buildPath + "/api/" + O.Name + "Login.go")
	check(err)
	defer f.Close()
...
func buildGoAPIEndPoint(buildPath string, O Object) {
	f, err := os.Create("./" + buildPath + "/api/" + O.Name + "EP.go")
	check(err)
	defer f.Close()
...
func strGoAPI_GETPaginated(O Object) string {
	return ""
}

func strGoAPI_GET(O Object) string {
...
func strGoAPI_POST(O Object) string {
	strP := "\treturn http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {\n"
...
func strGoAPI_PUT(O Object) string {
	strP := "\treturn http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {\n"
...
func strGoAPI_DELETE(O Object) string {
	strP := "\treturn http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {\n"
