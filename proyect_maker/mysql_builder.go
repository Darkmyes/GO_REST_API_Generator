...
func MySQLMakeCode(P Proyect, Obs []Object) {
	...
	_, err = f.WriteString(buildTables(Obs))

}

func buildDataBase(P Proyect) string {
...
func buildTables(Objs []Object) string {
	strP := ""
...
func buildTable(O Object) string {
	strP := "CREATE TABLE " + O.Tbl_Name + " (\n"
	strP += buildColumns(O.Columns)
	strP += buildFKs(O.Columns)
	strP += buildPKs(O.Columns)
	strP += ") ENGINE = INNODB;\n\n"
	return strP
}
...
func buildFKs(Cols []Column) string {
	strP := ""
	for _, C := range Cols {
		if C.Foreign_Key {
...
func buildPKs(Cols []Column) string {
	strP := "\tPRIMARY KEY ("
...
func buildIndexes(Tbl_Name string, Cols []Column) string {
